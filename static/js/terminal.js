	...
	if (_is_mobile) {
					...
					try {
						...
						if ((!(typeof str_command === "undefined")) && (str_command != '')) {
							...
							if ((str_command.toString().length == 1) || (str_command.toString() in window)) { // if it's a defined variable or function without ()
								...
								if (errorCount == 1) {
									errorResponse = 'Still not defined';
								}
								if (errorCount == 2) {
									errorResponse = 'Again?';
								}
		...
		$(document).keydown(function (e) {
			...
			if (((e.which == 38) && (kc_count < 2)) ||
				...
				((e.which == 66) && (kc_count == 8))) {
				resetkc = false;
				kc_count++;
			}
			if ((e.which == 65) && (kc_count == 9)) {
				kc_count++;
			}
			if (kc_count == 10) {
		...
		function newsletter() {
			window.open("https://blogs.kjscecodecell.com");
		}

		function feedback() {
			window.open("https://forms.gle/AHyUXnCyJYFxiPaA7");
		}
		...
		function likeasir(func_options) {
			...
			$('.avatar').each((index, element) => {
				...
				$(element).attr('src', src.replace('/shades/', '/normal/').replace('/likeasir/', '/normal/'));
			});
			if (func_options != "clear") {
				$('.avatar').each((index, element) => {
					var src = $(element).attr('src');
					$(element).attr('src', src.replace('/normal/', '/likeasir/'));
				});
		...
		function shades(func_options) {
			...
			if (func_options != "clear") {
				$('.avatar').each((index, element) => {
					var src = $(element).attr('src');
					$(element).attr('src', src.replace('/normal/', '/shades/'));
				});
